...
<?php 
	...
	if (is_page()) {
		...
		if (is_category()) {
			...
			if (count($disp) < 2) {
				// sometimes the unicode longer - ends up being used here, dunno why
				$disp = explode("&#8211;",single_cat_title('',false));
			}
			$barDisplay = $disp[0];
		...
		$currentPageURL = get_home_url().'/category/'.$bar.'_'.$slug[1].'/';	
		$slug = $slug[1];
	}
	
	global $url, $cat;
	$url = get_home_url().'/'.$bar;
	$cat = get_home_url().'/category/'.$bar.'_';
...
<header class="contain-to-grid" id="top">
	...
	<div class="row">	       
		<div class="small-8 medium-5 large-3 columns" id="logo">
			<a href="<?php echo get_home_url();?>"><img src="<?php echo get_template_directory_uri(); ?>/img/logo-<?php echo $bar?>.png" width="89" height="91"/></a>
		</div>

		<div class="small-10 medium-5 large-3 columns" id="ident">
			<p class="primary-color"><?php echo $barDisplay;?></p>
			<p class="sub"><a href="<?php echo get_home_url();?>">Choose a small bar</a></p>
